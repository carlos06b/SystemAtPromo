...
public class MenuRHFrame extends JFrame {
    ...
    private JPanel createSidebar(User user) {
        ...
        JLabel setor = new JLabel("Módulo RH");
        ...
        JLabel footer = new JLabel("Gestão de pessoas");
    ...
    private JPanel createContentPanel(User user) {
        ...
        title.setBounds(45, 35, 520, 40);
        panel.add(title);

        JLabel subtitle = new JLabel("Gerencie promotores e envie solicitações para análise do setor financeiro.");
        subtitle.setForeground(DARK_GRAY);
        subtitle.setFont(new Font("Segoe UI", Font.PLAIN, 15));
        subtitle.setBounds(48, 78, 620, 30);
        panel.add(subtitle);

        JPanel cardsPanel = new JPanel(new GridLayout(1, 2, 22, 22));
        cardsPanel.setBackground(LIGHT_GRAY);
        cardsPanel.setBounds(45, 145, 585, 160);

        cardsPanel.add(createMenuButton(
                "Promotores",
                "Cadastrar, listar, buscar, atualizar e inativar promotores",
                e -> new PromoterFrame()
        ));

        cardsPanel.add(createMenuButton(
                "Solicitações",
                "Criar e acompanhar solicitações enviadas ao financeiro",
        ...
        infoCard.setBorder(BorderFactory.createLineBorder(new Color(225, 225, 225)));
        infoCard.setBounds(45, 335, 585, 105);

        JLabel infoTitle = new JLabel("Fluxo principal do RH");
        infoTitle.setForeground(BLACK);
        infoTitle.setFont(new Font("Segoe UI", Font.BOLD, 16));
        infoTitle.setBounds(20, 15, 300, 25);
        infoCard.add(infoTitle);

        JLabel infoText = new JLabel("<html>Cadastre e mantenha os promotores atualizados. Quando houver bonificação, ajuda de custo, desconto, ASO ou EPI, crie uma solicitação para o financeiro aprovar.</html>");
        infoText.setForeground(DARK_GRAY);
        infoText.setFont(new Font("Segoe UI", Font.PLAIN, 13));
        infoText.setBounds(20, 43, 535, 45);
    ...
    private JButton createMenuButton(String title, String description, java.awt.event.ActionListener action) {
        JButton button = new JButton(
                "<html><b style='font-size:14px;'>" + title + "</b><br><span style='font-size:10px;'>"
